...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
	...
	int log_file_name_len = cycle->new_log->file->name.len+1;
	char str[log_file_name_len]; // 创建一个足够大的字符数组来存储unsigned char值和一个终结符'\0'
	int iii=0;
	for (iii = 0; iii < log_file_name_len; iii++){
				str[iii] = cycle->new_log->file->name.data[iii]; // 将unsigned char值复制到数组的第一个元素
	}
	str[log_file_name_len] = '\0';

    ngx_log_error(NGX_LOG_NOTICE, log, 0, "new_log_file_name: %s", str); // new_log_file_name: /usr/local/nginx/logs/error.log
    ngx_log_error(NGX_LOG_NOTICE, log, 0, "new_log_file_fd: %d", cycle->new_log->file->fd);
    ngx_log_error(NGX_LOG_NOTICE, log, 0, "new_log_file_name.len: %d", cycle->new_log->file->name.len);
    ...
    if (!ngx_test_config && cycle->log->file->fd != STDERR_FILENO) {
		...
		ngx_log_error(NGX_LOG_NOTICE, log, 0, "-- before dup2(), this will show on terminal, cycle->log->file->fd: %d",cycle->log->file->fd);

        if (dup2(cycle->log->file->fd, STDERR_FILENO) == -1) { // 文件描述符重定向
            ngx_log_error(NGX_LOG_EMERG, log, ngx_errno,
                          "dup2(STDERR) failed");
            /* fatal */
            exit(1);
        }
		ngx_log_error(NGX_LOG_NOTICE, log, 0, "-- after dup2(), this will be redirect to log file, cycle->log->file->fd: %d",cycle->log->file->fd);
    }
	...
	ngx_log_error(NGX_LOG_NOTICE, log, 0, "-- 11");
old_shm_zone_done:
